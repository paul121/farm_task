...
class TaskListTasks extends FormBase {
   ...
   *
   * @return string
   *   The title of the task in the plan.
   ...
   * @param \Drupal\plan\Entity\PlanInterface|null $plan
   *   (optional) The plan which contains the task list. Defaults to NULL.
   * @param int|null $delta
   *   (optional) The delta of the task list. Defaults to NULL.
   *
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($task_list->get('task')->referencedEntities() as $id => $task) {
      if ($form_state->hasValue(['tasks', $id])) {
        ...
        $component_config = $task->getComponentConfiguration();
        $form_display->setComponent($field_id, $component_config);
        $form_display->extractFormValues($task, $form['tasks'][$id], $form_state);
